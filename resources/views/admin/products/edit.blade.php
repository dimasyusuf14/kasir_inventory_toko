...
<div class="max-w-xl mx-auto">
    ...
    <form action="{{ route('admin.barang.update', $product->id) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')

        <div>
            <label class="block">Kode Barang</label>
            <input type="text" name="kode_barang" class="w-full border p-2 rounded bg-gray-100" value="{{ $product->kode_barang }}" readonly>
        </div>

        <div>
            <label class="block">Nama Barang</label>
            <input type="text" name="nama_barang" class="w-full border p-2 rounded" value="{{ $product->nama_barang }}" required>
        ...
        <div>
            ...
            <input type="number" name="stok" class="w-full border p-2 rounded" value="{{ $product->stok }}" required>
